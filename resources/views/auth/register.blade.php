...
<section class="py-20">
    <div class="max-w-md mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-xl shadow-md p-8">
            <h1 class="font-display text-3xl font-bold text-maisara-navy mb-6 text-center">{{ __('Client Registration') }}</h1>

            <form method="POST" action="{{ route('client.register.submit', ['locale' => app()->getLocale()]) }}">
                @csrf

                <div class="mb-4">
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-2">{{ __('Name') }}</label>
                    <input type="text" name="name" id="name" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-maisara-ember">
                </div>
